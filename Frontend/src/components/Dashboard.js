...
function Dashboard({ setUser }) {
  ...
  // Remove property states (moved to sidebar for admins)
      ...
      fetch("https://cp-check-submissions-dev-backend.onrender.com/api/recent-submissions", {
        ...
        .then((data) => {
          const completed = Array.from(
            new Set(
              data.filter((sub) => new Date(sub.submittedAt) >= new Date(loginTime))
  ...
  // ======================
  // 3) (Removed Remove button from property cards)
  // Remove property logic is now handled via the admin sidebar modal.
  // ======================
  ...
  // ======================
  // 4) Add Property Logic remains unchanged
  // ======================
  ...
  // ======================
  // 6) Submit new property logic (unchanged)
  ...
  // ======================
  // 8) Sorted Properties & Paging Logic (unchanged)
      ...
      <div className={`sidebar ${sidebarCollapsed ? "collapsed" : ""}`}>
        ...
        {!sidebarCollapsed && (
          <>
            ...
            <ul>
              {displayedProperties.map((prop) => (
                <li
                  ...
                  onClick={() => {
                    if (role === "admin") {
                      // For admin, navigate to access instructions directly
                      ...
                      switch (prop.orgType) {
                        ...
                        case "LTR":
                          ...
                          break;
                        case "STR":
                          // For STR users, show modal options
            ...
            {/* New section for My assignments for non-admin users */}
            {role !== "admin" && (
            ...
            {role === "admin" && (
              <div className="tools-section" style={{ marginBottom: "-10px" }}>
                ...
                <button
                  className="Admin-tools-secondary"
                  onClick={() => {
        ...
        {/* Modal for removing property (admin only) */}
        ...
        {/* STR Property Selection Modal for non-admin users */}
        {showModal && (
          <div className="modal-overlay">
            <div className="modal">
              ...
              {displayedProperties.map((prop) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      if (role === "admin") {
                        // For admin, clicking the card now navigates to Access Instructions.
                        navigate(`/access-instructions/${encodeURIComponent(prop.name)}`);
                      } else {
                        switch (prop.orgType) {
                          case "COM":
                  ...
                  >
                    ...
                    {/* For non-admins, show a Navigate button if lat/lng exist */}
