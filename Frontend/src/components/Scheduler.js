...
import withDragAndDrop from "react-big-calendar/lib/addons/dragAndDrop"; // Import DnD
...
function Scheduler() {
  ...
  const onEventDrop = ({ event, start, end }) => {
    const updatedAssignments = assignments.map((existingEvent) =>
  ...
  const onEventResize = ({ event, start, end }) => {
    ...
    fetch(`https://cp-check-submissions-dev-backend.onrender.com/api/assignments/${event._id}`, {
      ...
      .then((data) => {
        if (!data.success) {
          alert("Failed to resize assignment.");
        }
      })
      .catch((err) => console.error("Error resizing assignment:", err));
  };
