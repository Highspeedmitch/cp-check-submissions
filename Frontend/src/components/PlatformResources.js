...
export default function PlatformResources() {
  ...
  async function createPayoutBatch(event) {
    ...
    const completed = await run("batch", () => api.post("/api/platform-resources/payout-batches", {
      ...
      checkDate,
    }), "Gusto payout batch prepared. Submit it in Gusto, then record the payment-group UUID here.");
    if (completed) {
      setSelectedEarnings([]);
      setCheckDate("");
    }
  ...
  return (
    <div className="platform-resources">
          ...
          {data.resources.map((resource) => {
            ...
            return <article className="beta-panel platform-resource-card" key={resource._id}>
              ...
              <div className="beta-form-grid">
                ...
                <label className="beta-form-field full">Gusto contractor UUID<input value={edit.gustoContractorUuid || ""} placeholder="Stored after Gusto self-onboarding" onChange={(event) => setResourceEdits((current) => ({ ...current, [resource._id]: { ...edit, gustoContractorUuid: event.target.value } }))} /></label>
      ...
      <section className="beta-panel">
        <div className="beta-section-heading"><div><p className="beta-eyebrow">Gusto reconciliation</p><h2>Payout Batches</h2><p>Record Gusto’s payment-group UUID after submission, then mark the batch paid only after Gusto confirms funding.</p></div></div>
        {data.payoutBatches.length ? <div className="beta-resource-payout-list">{data.payoutBatches.map((batch) => <article key={batch._id}><div><strong>{batch.batchNumber}</strong><small>{new Date(batch.checkDate).toLocaleDateString()} · {batch.lines.length} contractor{batch.lines.length === 1 ? "" : "s"}</small></div><strong>{money(batch.totalAmountCents, batch.currency)}</strong><span className={`beta-status ${batch.status === "paid" ? "success" : "warning"}`}>{batch.status}</span>{batch.status === "ready" && <button className="beta-text-button" onClick={() => { const uuid = window.prompt("Enter the Gusto contractor payment group UUID after submitting this batch in Gusto:"); if (uuid?.trim()) run(`batch-${batch._id}`, () => api.post(`/api/platform-resources/payout-batches/${batch._id}/record-submission`, { gustoPaymentGroupUuid: uuid.trim() }), "Gusto submission recorded."); }}>Record Gusto Submission</button>}{batch.status === "submitted" && <button className="beta-text-button" onClick={() => window.confirm("Has Gusto confirmed this payment group as funded?") && run(`batch-${batch._id}`, () => api.post(`/api/platform-resources/payout-batches/${batch._id}/mark-paid`, {}), "Payout batch marked paid.")}>Mark Paid</button>}</article>)}</div> : <div className="beta-empty-state">No Gusto payout batches have been created.</div>}
      </section>
    </div>
  );
}
